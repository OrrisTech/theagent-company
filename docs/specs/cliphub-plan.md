# ClipHub: Marketplace for TAC Team Configurations

> The "app store" for whole-company AI teams — pre-built TAC configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for TAC-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | The Agent Company company (agents, projects, governance) |
...
A complete The Agent Company company configuration:
...
Single-agent configurations designed to plug into a TAC org:
...
Portable skill files that any The Agent Company agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with The Agent Company's skill loading system
...
interface Listing {
  ...
  theagentcompanyVersionMin: string;     // Minimum The Agent Company version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target The Agent Company company
...
| `POST` | `/api/listings/:id/install` | Install to The Agent Company company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target The Agent Company company (or create new)
  → ClipHub API calls TAC API to:
  ...
  → Redirect to The Agent Company dashboard with new team running
```
...
### 5.3 Creator: Export from TAC → Publish

```
Running The Agent Company company → "Export as Blueprint" (CLI or UI)
  → The Agent Company exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (The Agent Company brand blue/purple)
...
| Install flow | One-click deploy to The Agent Company company |
...
{
  "targetCompanyId": "uuid",         // Existing The Agent Company company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `tac-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as The Agent Company
- **Backend**: Node.js API (or extend TAC server)
- **Database**: Postgres (can share The Agent Company's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with The Agent Company auth (or OAuth2)

### 9.2 Integration with The Agent Company

ClipHub can be:
- **Option A**: A separate app that calls The Agent Company's API to install blueprints
- **Option B**: A built-in section of the TAC UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing TAC UI and API.
...
- [ ] Install flow: blueprint → The Agent Company company
