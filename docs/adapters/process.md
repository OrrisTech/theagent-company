...
- Running a Python script that calls the TAC API
...
1. The Agent Company spawns the configured command as a child process
2. Standard The Agent Company environment variables are injected (`TAC_AGENT_ID`, `TAC_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the TAC API and perform work.
