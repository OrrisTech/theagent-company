...
Build a custom adapter to connect The Agent Company to any agent runtime.
...
2. Build environment with `buildTacEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `theagentcompany run --watch` using `picocolors`.
...
Make The Agent Company skills discoverable to your agent runtime without writing to the agent's working directory:
