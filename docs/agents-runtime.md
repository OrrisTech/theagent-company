...
Audience: Operators setting up and running agents in The Agent Company

## 1. What this system does

Agents in The Agent Company do not run continuously.  
...
For `claude_local` and `codex_local`, The Agent Company assumes the CLI is already installed and authenticated on the host machine.
...
The Agent Company stores session IDs for resumable adapters.
...
The Agent Company pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. The Agent Company surfaces this as a warning in environment tests, not a hard error.
