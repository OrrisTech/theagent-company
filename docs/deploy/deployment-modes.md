...
The Agent Company supports two runtime modes with different security profiles.
...
```sh
pnpm theagentcompany allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, The Agent Company emits a one-time claim URL at startup:
...
```sh
TAC_DEPLOYMENT_MODE=authenticated pnpm theagentcompany run
```
