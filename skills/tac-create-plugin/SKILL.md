---
name: tac-create-plugin
description: >
  Create new TAC plugins with the current alpha SDK/runtime. Use when
...
Use this skill when the task is to create, scaffold, or document a TAC plugin.
...
node packages/plugins/create-tac-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the TAC repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.theagentcompany-sdk/`:

```bash
pnpm --filter @theagentcompany/create-tac-plugin build
node packages/plugins/create-tac-plugin/dist/index.js @acme/plugin-name \
...
- is installed into The Agent Company from an absolute local path during development
