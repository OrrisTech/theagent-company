...
ClipHub is the public registry where people share, discover, and download The Agent Company company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to The Agent Company what a package registry is to a programming language. The Agent Company already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in The Agent Company — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own TAC instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the TAC spec. It contains:
...
**Install (fresh start):**
```
theagentcompany install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local TAC instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within The Agent Company, export your company as a template, then publish:

```
theagentcompany export --template my-company
theagentcompany publish cliphub my-company
```
...
ClipHub is a **separate service** from TAC itself. The Agent Company is self-hosted; ClipHub is a hosted registry that TAC instances talk to.
...
| **TAC CLI** | `theagentcompany install`, `theagentcompany publish`, `theagentcompany cliphub sync` — built into The Agent Company |
| **TAC UI** | "Browse ClipHub" panel in the TAC web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with The Agent Company) |
| Backend | TypeScript + Hono (consistent with The Agent Company) |
| Database | PostgreSQL |
...
4. Run `theagentcompany install cliphub:acme/lean-saas-shop`
5. The Agent Company creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in The Agent Company until it works well
2. Export: `theagentcompany export --template my-agency`
3. Publish: `theagentcompany publish cliphub my-agency`
...
3. Install just that agent: `theagentcompany install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to The Agent Company

ClipHub is **not required** to use The Agent Company. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to The Agent Company what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `theagentcompany install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in The Agent Company web UI
- [ ] `theagentcompany cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with The Agent Company's own philosophy)
