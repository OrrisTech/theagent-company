...
This document defines the V1 design for a TAC adapter that integrates with
...
4. synthesized stdout events for TAC UI/CLI
...
3. **Agent callback auth to The Agent Company** uses a bootstrap exchange flow (no long-lived The Agent Company key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from TAC endpoints, not full SKILL.md prompt injection.
...
- `theagentcompanyPublicUrl` (optional override; else `TAC_PUBLIC_URL` env)
...
## The Agent Company Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `TAC_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.theagentcompanyPublicUrl`
2. `process.env.TAC_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived The Agent Company credentials in prompt text.

Flow:

1. Before launch/follow-up, The Agent Company mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
   - short TTL (for example 10 minutes)
2. Adapter includes only:
   - `theagentcompanyPublicUrl`
   - exchange endpoint path
   - bootstrap token
3. Cursor agent calls:
   - `POST /api/agent-auth/exchange`
4. The Agent Company validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all TAC API calls.
...
1. Prompt includes a compact instruction to fetch skills from TAC.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/tac`
   - `GET /api/skills/tac-create-agent` when needed
...
- resolve `theagentcompanyPublicUrl`
...
- include webhook object when enabled:
  - `url: <theagentcompanyPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active The Agent Company run context.
...
5. `theagentcompanyPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- theagentcompany public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from TAC skill endpoints |
| The Agent Company auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from TAC context.
3. Add richer PR metadata surfacing in TAC UI.
