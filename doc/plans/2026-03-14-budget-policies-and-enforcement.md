...
The Agent Company already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model The Agent Company should implement next.

## Product Goals

The Agent Company should let operators:
...
For the next implementation pass, The Agent Company should enforce these defaults:
...
The Agent Company should separate subscription-included usage from billed spend:
...
The Agent Company should have two threshold classes:
...
The Agent Company must know that a project is budget-paused and enforce it.
...
The Agent Company must also block execution before new work starts.
