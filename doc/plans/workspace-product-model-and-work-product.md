...
The Agent Company needs to support two very different but equally valid ways of working:
...
Today, The Agent Company already has the beginnings of this model:
...
- making The Agent Company feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing The Agent Company users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted TAC deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning The Agent Company into a full code review product
...
This object must be recorded explicitly so that The Agent Company can:
...
The Agent Company should treat PRs as a type of work product linked back to:
...
The Agent Company already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted The Agent Company must be a first-class deployment mode

The Agent Company cannot assume that it is running on the same machine as the code.

In cloud deployments, The Agent Company may:
...
- `Runtime service`: a process or service The Agent Company owns or tracks for a workspace
...
It also lets cloud-hosted TAC deployments point at codebases and remotes without pretending the TAC host has direct filesystem access.
...
This lets The Agent Company support:
...
Without an explicit `execution workspace` record, The Agent Company has nowhere stable to attach:
...
The Agent Company needs a single place to show:
...
- The Agent Company-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without The Agent Company owning the execution host
...
## 1. The Agent Company host and execution host must be decoupled

The Agent Company may run:
...
without the TAC host itself running local git or local preview processes.
...
The Agent Company should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for TAC.
...
The Agent Company should keep workspace automation centered on a small set of hooks:
...
The Agent Company should provide the equivalent visibility in the project `Code` surface:
...
The Agent Company should treat execution workspaces as the natural home for:
...
The Agent Company should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
The Agent Company should continue treating cleanup policy as part of the core workspace model:
...
The Agent Company should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the TAC host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

The Agent Company should borrow Worktrunk's good execution patterns while keeping the broader The Agent Company model:
...
This is how The Agent Company supports:
...
That keeps The Agent Company focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted TAC deployments remain first-class

That is a better fit for TAC than either extreme:
