...
`PAP-447` asks how The Agent Company should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same TAC instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into The Agent Company:

- some operators use The Agent Company to manage The Agent Company and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as The Agent Company
- Claude and Codex expose different built-in affordances, so The Agent Company should not overfit to one tool

## Core Product Decision

The Agent Company should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by TAC
...
- The Agent Company should not assume that every adapter can see or use a host filesystem path directly
...
If The Agent Company makes worktrees agent-first, it will blur:
...
- The Agent Company core resolves and records execution workspace state
...
That gives The Agent Company room to support:
...
- The Agent Company handles workspace runtime services under the hood using project/workspace policy
...
Once The Agent Company is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So The Agent Company should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that The Agent Company cannot infer
...
- when issue moves to done, should The Agent Company auto-commit?
- should it auto-push?
...
The Agent Company already has the right foundation for a project-first model.
...
For server-spawned local adapters, The Agent Company already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that The Agent Company should mirror directly

Implication:

- for `codex_local`, The Agent Company should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but The Agent Company should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, The Agent Company should separate:
...
The Agent Company should be able to express intentions such as:
...
These are not specific to The Agent Company. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".theagentcompany/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a TAC-specific `devServer` flag.
...
The Agent Company should distinguish between:
...
The Agent Company should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
### The Agent Company Core

The Agent Company core should:
...
The Agent Company core should not:
...
In addition to the local git helper, The Agent Company should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to The Agent Company
...
For runtime services specifically, The Agent Company should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as The Agent Company-only dev-server flags
...
3. Inject resolved execution workspace details into `context.tacWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / TAC instance can be reused through env/config injection when appropriate
...
- The Agent Company can decide whether to reuse or start a fresh service deterministically
...
3. Test against The Agent Company-on-The Agent Company workflows first.
...
7. Workspace runtime services are modeled generically, not as The Agent Company-specific dev-server toggles.
