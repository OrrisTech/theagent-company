...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/theagentcompany-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `TAC_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `TAC_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - The Agent Company may create host-local worktrees, processes, and paths
- `cloud`
  - The Agent Company should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `theagentcompany | github | vercel | s3 | custom`
...
- use `TAC_EXECUTION_TOPOLOGY` as a defaulting guardrail
