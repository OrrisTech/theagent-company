...
Add a single storage subsystem for TAC that supports:
...
- API serves attachment bytes through authenticated The Agent Company endpoints.
...
- Default local path is under instance root: `~/.theagentcompany/instances/<instanceId>/data/storage`.
...
- `theagentcompany onboard` writes a valid `storage` config block by default.
- `theagentcompany configure --section storage` can switch between local and s3 modes.
...
- `theagentcompany doctor` reports actionable storage status.
