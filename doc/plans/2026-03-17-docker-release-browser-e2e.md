...
Today release smoke testing for published The Agent Company packages is manual and shell-driven:
...
- Docker
- `npx theagentcompany@canary`
- `npx theagentcompany@latest`
...
- runs `theagentcompany@${TACAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@theagentcompany.local`
- password: `theagentcompany-smoke-password`
...
- `theagentcompany_version=canary`
...
- `theagentcompany_version=latest`
...
- the suite passes locally against both:
  - `TACAI_VERSION=canary`
  - `TACAI_VERSION=latest`
...
1. A published `theagentcompany@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `theagentcompany@latest` Docker install can be smoke-tested by Playwright in CI.
