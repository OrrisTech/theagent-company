...
Agents need API keys to authenticate with The Agent Company. The current approach
...
**Trust model:** The adapter process runs on the same machine as the TAC
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** The Agent Company generates a token and passes it directly to the agent
...
`TAC_API_URL`, `TAC_AGENT_ID`, `TAC_COMPANY_ID`. We need to
add a `TAC_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a TAC CLI
...
```
theagentcompany auth login
# Opens browser -> user confirms -> token stored at ~/.theagentcompany/credentials
```
...
managed by the TAC server itself.
...
1. A company admin (user or agent) generates an **invite URL** from TAC.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The The Agent Company SKILL.md (or a link to it)
   - What information The Agent Company needs from the agent (e.g. webhook URL, adapter
...
5. The Agent Company stores the pending registration.
...
7. On approval, The Agent Company provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** The Agent Company issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.theagentcompany.ing/skills/tac/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How The Agent Company should send you heartbeats",
...
1. Generate a TAC invite link for the company.
...
4. A The Agent Company company member approves the new agent.
5. The Agent Company begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `TAC_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `theagentcompany auth login` for developer-managed remote agents.                                      |
