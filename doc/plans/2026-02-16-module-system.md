# The Agent Company Module System

## Overview

The Agent Company's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends The Agent Company's API, UI, and data model | Yes |
...
modules/
  observability/
    theagentcompany.module.json     # manifest (required)
    src/
...
### Manifest (`theagentcompany.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@theagentcompany/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@theagentcompany/core";
...
```
1. Scan modules/ directory for theagentcompany.module.json manifests
...
```jsonc
// theagentcompany.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@theagentcompany/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@theagentcompany/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:theagentcompany/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.theagentcompany.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm theagentcompany store list                    # browse available modules and templates
pnpm theagentcompany store install <module-id>     # install a module
pnpm theagentcompany store import <template-id>    # import a company template
pnpm theagentcompany store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one TAC instance | `server:started` |
...
4. **Module config** — `theagentcompany.config.json` with per-module config, env var interpolation
...
10. **`@theagentcompany/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
