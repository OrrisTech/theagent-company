...
- New config/env settings:
  - `TAC_AGENT_JWT_SECRET`
  - `TAC_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `TAC_AGENT_JWT_ISSUER` (default: `theagentcompany`)
  - `TAC_AGENT_JWT_AUDIENCE` (default: `theagentcompany-api`)
...
1. In:
   ...
   inject `TAC_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `TAC_API_KEY`, do not overwrite it.
- Continue injecting:
  - `TAC_AGENT_ID`
  - `TAC_COMPANY_ID`
  - `TAC_API_URL`
...
- Local adapters authenticate without manual `TAC_API_KEY` config.
