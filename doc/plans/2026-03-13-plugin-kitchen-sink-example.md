...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented TAC plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside The Agent Company.
...
- package: `@theagentcompany/plugin-kitchen-sink-example`
- plugin id: `theagentcompany.kitchen-sink-example` or `theagentcompany-kitchen-sink-example`

Recommendation: use `theagentcompany-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core The Agent Company event such as `issue.created`
...
Recommendation: default to a TAC-local endpoint or a stable public echo endpoint to avoid flaky docs.
