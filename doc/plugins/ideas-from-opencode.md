...
The Agent Company V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a TAC-shaped design.

Assumption for this document: The Agent Company is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class The Agent Company objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into The Agent Company.

The main conclusion is:

- The Agent Company should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- The Agent Company should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- The Agent Company should use multiple extension classes instead of one generic plugin bag:
...
If The Agent Company does this well, the examples you listed become straightforward:
...
Relevant The Agent Company files reviewed for current extension seams:
...
It is too dangerous for TAC core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for TAC — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
The Agent Company should absolutely do this.
...
The Agent Company should copy this discipline.
...
The Agent Company should adopt this style for plugin actions, automations, and UI schemas.
...
The Agent Company can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for TAC too.

## What The Agent Company Should Not Copy Directly
...
The Agent Company is a control plane for an operator-managed instance with company objects.
...
The Agent Company is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

The Agent Company should not auto-load arbitrary code from a workspace repo like `.theagentcompany/plugins` or project directories.
...
For The Agent Company, equivalent hooks into:
...
The Agent Company should not allow plugins to silently replace:
...
For The Agent Company it would be risky because it combines:
...
The Agent Company should require an explicit operator install step.

## Why The Agent Company Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | The Agent Company |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted TAC instance |
...
That means The Agent Company should borrow the good ideas from `opencode` but use a stricter architecture.

## The Agent Company Already Has Useful Pre-Plugin Seams

The Agent Company has several extension-like seams already:
...
This is good news.
The Agent Company does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended The Agent Company Plugin Model
...
The Agent Company already has this pattern implicitly:
...
- `plugin` means integration code that talks to The Agent Company through a typed plugin protocol and capability model
...
- invoke explicit The Agent Company actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@theagentcompany/plugin-sdk/ui`) with design tokens and common components.
...
The Agent Company should treat plugin installation as a global instance-level action.

Examples:

- install `@theagentcompany/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over The Agent Company objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

The Agent Company already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for TAC too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@theagentcompany/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@theagentcompany/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running TAC instance.

A `create-tac-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how The Agent Company achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the TAC server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. The Agent Company's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@theagentcompany/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@theagentcompany/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For The Agent Company

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@theagentcompany/plugin-sdk";

export default definePlugin({
  id: "@theagentcompany/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit The Agent Company entities
    });
    ...
    ctx.events.on("plugin.@theagentcompany/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@theagentcompany/plugin-sdk/ui";
...
Any TAC plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific The Agent Company objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@theagentcompany/plugin-linear`)
...
- reuse core tables when the data is clearly part of The Agent Company itself
...
If a concept is becoming part of The Agent Company's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of The Agent Company itself
...
### 3. Scope plugin data to The Agent Company objects before adding custom schemas

A lot of plugin data naturally hangs off existing The Agent Company objects:
...
If The Agent Company eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@theagentcompany/plugin-workspace-files`
...
Package idea: `@theagentcompany/plugin-terminal`
...
Package idea: `@theagentcompany/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving The Agent Company.
- Board reviews the diff after a run without leaving The Agent Company.
...
- optional `events.emit` (e.g. `plugin.@theagentcompany/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@theagentcompany/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@theagentcompany/plugin-linear`

This plugin syncs The Agent Company work with Linear. It is useful for:

- importing backlog from Linear
- linking The Agent Company issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between The Agent Company and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into The Agent Company.
- The Agent Company issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@theagentcompany/plugin-github-issues`

This plugin syncs The Agent Company issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to The Agent Company issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with The Agent Company task governance
...
- Plugin settings:
  ...
  - whether new The Agent Company issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active The Agent Company issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into The Agent Company.
- GitHub webhooks update status/comment state in The Agent Company.
- A PR is linked back to the TAC issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@theagentcompany/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@theagentcompany/plugin-grafana`

This plugin surfaces external metrics and dashboards inside The Agent Company. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating The Agent Company issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full TAC plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the TAC dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a TAC issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of The Agent Company core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@theagentcompany/plugin-runtime-processes`
...
Package idea: `@theagentcompany/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into The Agent Company. It is useful for:
...
- Failed payment spikes or churn events can generate The Agent Company issues for follow-up.
...
- Stripe data should stay additive to The Agent Company core
...
- plugin UI bundle loading, host bridge, `@theagentcompany/plugin-sdk/ui`
...
- `@theagentcompany/plugin-test-harness` and `create-tac-plugin` starter template
...
The Agent Company should not implement "an OpenCode-style generic in-process hook system."
The Agent Company should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In The Agent Company
...
8. Ship `@theagentcompany/plugin-test-harness` and `create-tac-plugin` starter template.
