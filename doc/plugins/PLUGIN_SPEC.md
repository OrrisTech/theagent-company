# The Agent Company Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for TAC's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main The Agent Company app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary The Agent Company HTTP APIs directly.
...
TAC plugin design is based on the following assumptions:

1. The Agent Company is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core The Agent Company business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by TAC core.
...
2. Let plugins add major capabilities without editing The Agent Company core.
...
4. Depend on project-local plugin folders such as `.theagentcompany/plugins`.
...
The single TAC deployment an operator installs and controls.

### 5.2 Company

A first-class The Agent Company business object inside the instance.
...
A trusted in-process extension loaded directly by TAC core.
...
An installable instance-wide extension package loaded through the TAC plugin runtime.
...
The Agent Company has two extension classes.
...
The Agent Company already has a concrete workspace model:
...
Plugins live under the TAC instance directory.

Suggested layout:

- `~/.theagentcompany/instances/default/plugins/package.json`
- `~/.theagentcompany/instances/default/plugins/node_modules/`
- `~/.theagentcompany/instances/default/plugins/.cache/`
- `~/.theagentcompany/instances/default/data/plugins/<plugin-id>/`
...
The Agent Company should add CLI commands:

- `pnpm theagentcompany plugin list`
- `pnpm theagentcompany plugin install <package[@version]>`
- `pnpm theagentcompany plugin uninstall <plugin-id>`
- `pnpm theagentcompany plugin upgrade <plugin-id> [version]`
- `pnpm theagentcompany plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@theagentcompany/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```json
{
  "name": "@theagentcompany/plugin-linear",
  "version": "0.1.0",
  "tacPlugin": {
...
```ts
export interface TacPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that The Agent Company agents can use during runs.
...
- TAC server starts one worker process per installed plugin
...
Receives one typed The Agent Company domain event.
...
export function definePlugin(definition: PluginDefinition): TacPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: TacPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@theagentcompany/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@theagentcompany/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@theagentcompany/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@theagentcompany/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@theagentcompany/plugin-sdk/ui`
...
The `@theagentcompany/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the TAC secret provider system rather than a plain text input
...
1. Core TAC data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing The Agent Company objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual The Agent Company product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if The Agent Company later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing The Agent Company secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm theagentcompany plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the TAC server. This is a normative requirement, not optional.
...
### 27.1 `@theagentcompany/plugin-test-harness`
...
```ts
import { createTestHarness } from "@theagentcompany/plugin-test-harness";
...
For developing a plugin against a running TAC instance:

- The operator installs the plugin from a local path: `pnpm theagentcompany plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-tac-plugin`) that scaffolds:

- `package.json` with correct `tacPlugin` keys
...
- `@theagentcompany/plugin-workspace-files`
- `@theagentcompany/plugin-terminal`
- `@theagentcompany/plugin-git`
- `@theagentcompany/plugin-linear`
- `@theagentcompany/plugin-github-issues`
- `@theagentcompany/plugin-grafana`
- `@theagentcompany/plugin-runtime-processes`
- `@theagentcompany/plugin-stripe`
...
- `@theagentcompany/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@theagentcompany/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@theagentcompany/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@theagentcompany/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@theagentcompany/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@theagentcompany/plugin-sdk/ui`
...
- `@theagentcompany/plugin-test-harness`
- `create-tac-plugin` starter template
...
The Agent Company should not implement a generic in-process hook bag modeled directly after local coding tools.

The Agent Company should implement:
...
That is the complete target design for the TAC plugin system.
