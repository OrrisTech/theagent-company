...
This guide describes the current, implemented way to create a TAC plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main The Agent Company app.
...
```bash
pnpm --filter @theagentcompany/create-tac-plugin build
node packages/plugins/create-tac-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the TAC repo:

```bash
pnpm --filter @theagentcompany/create-tac-plugin build
node packages/plugins/create-tac-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@theagentcompany/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@theagentcompany/plugin-sdk` from the local The Agent Company checkout into a `.theagentcompany-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into The Agent Company from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@theagentcompany/plugin-sdk/ui`
