...
This spec defines how The Agent Company actually runs agents while staying runtime-agnostic.
...
1. The Agent Company is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, The Agent Company manages lifecycle, and on exit The Agent Company parses JSON output and updates state.
...
```json
{
  "runLogStore": {
    ...
    "bucket": "theagentcompany-run-logs",
    "prefix": "runs/",
    "compress": true,
...
- `run.startedAt`
- `heartbeat.reason`
- `theagentcompany.skill` (shared The Agent Company skill text block)
...
2. Preferred transport is env vars (`TAC_*`) injected at runtime.
