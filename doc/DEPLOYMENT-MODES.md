...
The Agent Company supports two runtime modes:
...
When running `authenticated` mode, if the only instance admin is `local-board`, The Agent Company emits a startup warning with a one-time high-entropy claim URL.
