...
TAC_HOST_PORT="${TAC_HOST_PORT:-3100}"
TAC_HOST_FROM_CONTAINER="${TAC_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.tac-openclaw.override.yml"
...
detect_tac_base_url() {
  ...
  for candidate in "$TAC_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${TAC_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${TAC_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
tac_base_url="$(detect_tac_base_url || true)"
...
The Agent Company URL for OpenClaw container:
EOF
  if [[ -n "$tac_base_url" ]]; then
    cat <<EOF
  $tac_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${TAC_HOST_PORT}
  (Do not use http://127.0.0.1:${TAC_HOST_PORT} inside the container.)
  If The Agent Company rejects the host, run on host machine:
    pnpm theagentcompany allowed-hostname host.docker.internal
  Then restart The Agent Company and re-run this script.
EOF
  fi
  cat <<EOF
