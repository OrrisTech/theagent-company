...
COMPANY_ID="${COMPANY_ID:-${TAC_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-tac-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-tac-openclaw-smoke}"
...
Provide one of:
  TAC_AUTH_HEADER=\"Bearer <board-token>\"
  TAC_COOKIE=\"<board-session-cookie>\"
...
log "checking The Agent Company health"
...
assert_status "200"
if ! grep -q "The Agent Company OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.tac.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
