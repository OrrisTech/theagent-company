...
Environment overrides:
  ...
  PC_DATA=/tmp/data           TAC data dir to use
  TAC_HOST=127.0.0.1    Host passed to the onboarded server
  TAC_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/tac-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/tac-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export TAC_HOST="${TAC_HOST:-127.0.0.1}"
export TAC_PORT="${TAC_PORT:-3100}"
export TAC_OPEN_ON_LISTEN="${TAC_OPEN_ON_LISTEN:-false}"
...
echo "TAC_HOST: $TAC_HOST"
echo "TAC_PORT: $TAC_PORT"
...
env \
  ...
  HOST="$TAC_HOST" \
  PORT="$TAC_PORT" \
  TAC_OPEN_ON_LISTEN="$TAC_OPEN_ON_LISTEN" \
  pnpm theagentcompany onboard --yes --data-dir "$PC_DATA"
