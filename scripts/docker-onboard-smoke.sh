...
IMAGE_NAME="${IMAGE_NAME:-tac-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
TAC_VERSION="${TAC_VERSION:-latest}"
...
TAC_DEPLOYMENT_MODE="${TAC_DEPLOYMENT_MODE:-authenticated}"
TAC_DEPLOYMENT_EXPOSURE="${TAC_DEPLOYMENT_EXPOSURE:-private}"
TAC_PUBLIC_URL="${TAC_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@tac.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-tac-smoke-password}"
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e TAC_DEPLOYMENT_MODE="$TAC_DEPLOYMENT_MODE" \
      -e TAC_DEPLOYMENT_EXPOSURE="$TAC_DEPLOYMENT_EXPOSURE" \
      -e TAC_PUBLIC_URL="$TAC_PUBLIC_URL" \
      -e TAC_HOME="/tac" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "theagentcompany@${TAC_VERSION}" auth bootstrap-ceo --data-dir "$TAC_HOME" --base-url "$TAC_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$TAC_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$TAC_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$TAC_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$TAC_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$TAC_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$TAC_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg TAC_VERSION="$TAC_VERSION" \
...
echo "    Public URL: $TAC_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $TAC_DEPLOYMENT_MODE/$TAC_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/tac-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$TAC_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $TAC_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$TAC_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $TAC_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
