#!/usr/bin/env bash
#
# Kill all local The Agent Company dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all TAC dev processes
...
# Collect PIDs of node processes running from any TAC directory.
# Matches paths like /Users/*/theagent-company/... or /Users/*/tac/... or /Users/*/tac-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/(tac|theagent-company)(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No The Agent Company dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} The Agent Company dev process(es):"
echo ""
