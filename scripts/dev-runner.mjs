...
const env = {
  ...process.env,
  TAC_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "watch") {
  env.TAC_MIGRATION_PROMPT ??= "never";
  env.TAC_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.TAC_DEPLOYMENT_MODE = "authenticated";
  env.TAC_DEPLOYMENT_EXPOSURE = "private";
  env.TAC_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[tac] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[tac] dev mode: local_trusted (default)");
}
...
async function maybePreflightMigrations() {
  ...
  const status = await runPnpm(
    ["--filter", "@theagentcompany/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[tac] Command failed with code ${status.code}: pnpm --filter @theagentcompany/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[tac] migration-status returned invalid JSON payload\n",
    );
  ...
  const autoApply = env.TAC_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    process.stderr.write(
      `[tac] Pending migrations detected (${formatPendingMigrationSummary(payload.pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[tac] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@theagentcompany/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[tac] plugin sdk build failed");
...
const child = spawn(
  pnpmBin,
  ["--filter", "@theagentcompany/server", serverScript, ...forwardedArgs],
