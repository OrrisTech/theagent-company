...
function isWorktreeMode(env: NodeJS.ProcessEnv): boolean {
  return TRUTHY_ENV_RE.test(env.TAC_IN_WORKTREE ?? "");
}

function resolveWorktreeCodexHomeDir(env: NodeJS.ProcessEnv): string | null {
  if (!isWorktreeMode(env)) return null;
  const tacHome = nonEmpty(env.TAC_HOME);
  if (!tacHome) return null;
  const instanceId = nonEmpty(env.TAC_INSTANCE_ID);
  if (instanceId) {
    return path.resolve(tacHome, "instances", instanceId, "codex-home");
  }
  return path.resolve(tacHome, "codex-home");
}
...
export async function prepareWorktreeCodexHome(
  ...
  await onLog(
    "stdout",
    `[tac] Using worktree-isolated Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
