...
Use when:
- You want The Agent Company to run Cursor Agent CLI locally as the agent runtime
...
- The Agent Company auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$tac" and related skills on local runs.
- The Agent Company auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
