...
Use when:
- You want The Agent Company to run Pi (the AI coding agent) locally as the agent runtime
...
- The Agent Company requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/tac-sessions/ and resumed with --session.
