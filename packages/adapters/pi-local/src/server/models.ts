...
import type { AdapterModel } from "@theagentcompany/adapter-utils";
import { asString, runChildProcess } from "@theagentcompany/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.TAC_PI_COMMAND === "string" &&
    process.env.TAC_PI_COMMAND.trim().length > 0
      ? process.env.TAC_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["TAC_", "npm_", "NPM_"] as const;
