...
## Scope
This plan is now **gateway-only**. The Agent Company supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses The Agent Company skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a TAC task.
...
```bash
pnpm --filter @theagentcompany/server test:run -- openclaw-gateway
pnpm --filter @theagentcompany/server typecheck
pnpm --filter @theagentcompany/ui typecheck
pnpm --filter theagentcompany typecheck
```
