...
import { asNumber, asString, buildTacEnv, parseObject } from "@theagentcompany/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "tac";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "tac";
  if (input.strategy === "run") return `tac:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `tac:issue:${input.issueId}`;
  return fallback;
}
...
function resolveTacApiUrlOverride(value: unknown): string | null {
...
function buildTacEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const tacApiUrlOverride = resolveTacApiUrlOverride(ctx.config.tacApiUrl);
  const tacEnv: Record<string, string> = {
    ...buildTacEnv(ctx.agent),
    TAC_RUN_ID: ctx.runId,
  };

  if (tacApiUrlOverride) {
    tacEnv.TAC_API_URL = tacApiUrlOverride;
  }
  if (wakePayload.taskId) tacEnv.TAC_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) tacEnv.TAC_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) tacEnv.TAC_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) tacEnv.TAC_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) tacEnv.TAC_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    tacEnv.TAC_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return tacEnv;
}

function buildWakeText(payload: WakePayload, tacEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/tac-claimed-api-key.json";
  const orderedKeys = [
    "TAC_RUN_ID",
    "TAC_AGENT_ID",
    "TAC_COMPANY_ID",
    ...
    "TAC_APPROVAL_ID",
    "TAC_APPROVAL_STATUS",
    "TAC_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = tacEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = tacEnv.TAC_API_URL ?? "<set TAC_API_URL>";

  const lines = [
    "The Agent Company wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `TAC_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load TAC_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $TAC_API_KEY on every API call.",
    "- Use X-Tac-Run-Id: $TAC_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: TAC_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$TAC_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$TAC_COMPANY_ID/issues?assigneeAgentId=$TAC_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardTacPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  tacEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateTac = parseObject(payloadTemplate.tac);
  const workspace = asRecord(ctx.context.tacWorkspace);
  const workspaces = Array.isArray(ctx.context.tacWorkspaces)
    ? ctx.context.tacWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.tacRuntimeServiceIntents)
    ? ctx.context.tacRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardTac: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: tacEnv.TAC_API_URL ?? null,
  };

  if (workspace) {
    standardTac.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardTac.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardTac.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "tac-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const tacEnv = buildTacEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, tacEnv);
  ...
  const tacPayload = buildStandardTacPayload(ctx, wakePayload, tacEnv, payloadTemplate);
