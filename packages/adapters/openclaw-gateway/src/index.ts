...
Use when:
- You want The Agent Company to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the TAC server.
...
- workspaceRuntime (object, optional): desired runtime service intents; The Agent Company forwards these in a standardized tac.workspaceRuntime block for remote execution environments
- timeoutSec (number, optional): adapter timeout in seconds (default 120)
- waitTimeoutMs (number, optional): agent.wait timeout override (default timeoutSec * 1000)
- autoPairOnFirstConnect (boolean, optional): on first "pairing required", attempt device.pair.list/device.pair.approve via shared auth, then retry once (default true)
- tacApiUrl (string, optional): absolute TAC base URL advertised in wake text

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default tac)

Standard outbound payload additions:
- tac (object): standardized TAC context added to every gateway agent request
- tac.workspace (object, optional): resolved execution workspace for this run
- tac.workspaces (array, optional): additional workspace hints The Agent Company exposed to the run
- tac.workspaceRuntime (object, optional): normalized runtime service intent config for the workspace
