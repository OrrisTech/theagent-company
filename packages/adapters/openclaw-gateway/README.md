...
This document describes how `@theagentcompany/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into The Agent Company logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (The Agent Company `runId`)
