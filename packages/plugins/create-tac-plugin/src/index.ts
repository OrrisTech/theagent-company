...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".tac-sdk");
...
/**
 * Generate a complete TAC plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A TAC plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "tac-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@theagentcompany/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@theagentcompany/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@theagentcompany/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@theagentcompany/plugin-sdk\` and \`@theagentcompany/shared\` from a local The Agent Company checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.tac-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-tac-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <tac-sdk-path>]");
    process.exit(1);
  }
