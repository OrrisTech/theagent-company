/**
 * `@theagentcompany/plugin-sdk` — TAC plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@theagentcompany/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@theagentcompany/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @theagentcompany/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@theagentcompany/plugin-sdk) for all plugin authoring needs.
export type {
  TacPluginManifestV1,
 ...
 * @example
 * ```ts
 * import { z } from "@theagentcompany/plugin-sdk";
 *
