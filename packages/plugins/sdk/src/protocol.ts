 ...
 * channel.
 *
 * The TAC plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@theagentcompany/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all The Agent Company messages.
 */
...
/**
 * TAC plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this TAC instance. */
    instanceId: string;
    /** Semver version of the running The Agent Company host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
