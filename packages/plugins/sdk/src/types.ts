/**
 * Core types for the TAC plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@theagentcompany/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @theagentcompany/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of The Agent Company object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the TAC UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit The Agent Company domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core The Agent Company domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the TAC secret provider
...
export interface PluginSecretsClient {
   ...
   * in the TAC secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@theagentcompany/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@theagentcompany/plugin-sdk";
 *
