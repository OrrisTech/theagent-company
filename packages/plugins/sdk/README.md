# `@theagentcompany/plugin-sdk`

Official TypeScript SDK for TAC plugin authors.

- **Worker SDK:** `@theagentcompany/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@theagentcompany/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@theagentcompany/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@theagentcompany/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@theagentcompany/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@theagentcompany/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@theagentcompany/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@theagentcompany/plugin-sdk/ui/hooks` | Hooks only |
| `@theagentcompany/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@theagentcompany/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@theagentcompany/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@theagentcompany/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@theagentcompany/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@theagentcompany/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @theagentcompany/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main The Agent Company app. They can call ordinary The Agent Company HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the TAC instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@theagentcompany/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@theagentcompany/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core The Agent Company information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@theagentcompany/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@theagentcompany/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@theagentcompany/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@theagentcompany/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@theagentcompany/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@theagentcompany/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@theagentcompany/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@theagentcompany/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@theagentcompany/plugin-sdk/ui";
...
import {
  useHostContext,
  usePluginAction,
} from "@theagentcompany/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@theagentcompany/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@theagentcompany/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@theagentcompany/plugin-sdk/bundlers";
...
```bash
theagentcompany-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@theagentcompany/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__theagentcompany__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__theagentcompany__/events` streams `reload` SSE events on UI build changes
