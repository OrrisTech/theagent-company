...
Example TAC plugin that demonstrates:
...
```bash
pnpm --filter @theagentcompany/plugin-file-browser-example build
pnpm theagentcompany plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm theagentcompany plugin uninstall theagentcompany-file-browser-example --force
```
...
- Optional: use `theagentcompany-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
