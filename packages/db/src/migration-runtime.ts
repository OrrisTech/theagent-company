...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://tac:tac@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "tac");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://tac:tac@127.0.0.1:${preferredPort}/tac`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://tac:tac@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "tac");
    return {
      connectionString: `postgres://tac:tac@127.0.0.1:${port}/tac`,
  ...
  const adminConnectionString = `postgres://tac:tac@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "tac");

  return {
    connectionString: `postgres://tac:tac@127.0.0.1:${selectedPort}/tac`,
