...
function resolveTacInstanceId(): string {
  const raw = process.env.TAC_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveTacHomeDir(), "instances", resolveTacInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveTacHomeDir(), "instances", resolveTacInstanceId(), "data", "backups");
}
