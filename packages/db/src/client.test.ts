...
async function createTempDatabase(): Promise<string> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "tac-db-client-"));
  ...
  const adminUrl = `postgres://tac:tac@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminUrl, "tac");
