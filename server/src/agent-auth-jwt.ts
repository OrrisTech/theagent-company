...
function jwtConfig() {
  const secret = process.env.TAC_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.TAC_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.TAC_AGENT_JWT_ISSUER ?? "tac",
    audience: process.env.TAC_AGENT_JWT_AUDIENCE ?? "tac-api",
  };
}
