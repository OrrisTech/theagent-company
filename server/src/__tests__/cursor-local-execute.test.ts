...
import { execute } from "@theagentcompany/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects TAC env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "tac-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the TAC heartbeat.");
      ...
      expect(capture.prompt).toContain("The Agent Company runtime note:");
      expect(capture.prompt).toContain("TAC_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "tac-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
