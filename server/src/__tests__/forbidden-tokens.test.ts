...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "tac", LOGNAME: "tac", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "tac" }),
      },
    );

    expect(tokens).toEqual(["tac", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "tac" }, {
        userInfo: () => ({ username: "tac" }),
      });

      expect(tokens).toEqual(["tac", "custom-token"]);
    } finally {
