...
import type { WorkspaceOperation } from "@theagentcompany/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tac-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "tac@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "The Agent Company Test"]);
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".tac", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$TAC_WORKSPACE_BRANCH\" > .tac-provision-branch",
        "printf '%s\\n' \"$TAC_WORKSPACE_BASE_CWD\" > .tac-provision-base",
        "printf '%s\\n' \"$TAC_WORKSPACE_CREATED\" > .tac-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".tac-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".tac-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".tac-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".tac-provision-created"), "utf8")).resolves.toBe("false\n");
  });
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tac-runtime-workspace-"));
  ...
  it("does not leak parent TAC instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tac-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "tacConfig: process.env.TAC_CONFIG ?? null,",
          "tacHome: process.env.TAC_HOME ?? null,",
          "tacInstanceId: process.env.TAC_INSTANCE_ID ?? null,",
    ...
    process.env.TAC_CONFIG = "/tmp/base-tac-config.json";
    process.env.TAC_HOME = "/tmp/base-tac-home";
    process.env.TAC_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/tac";
    ...
    expect(captured.tacConfig).toBeNull();
    expect(captured.tacHome).toBeNull();
    expect(captured.tacInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "tac-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "tac-runtime-sibling-"));
