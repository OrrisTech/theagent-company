...
<head>
    <!-- TAC_RUNTIME_BRANDING_START -->
    <!-- TAC_RUNTIME_BRANDING_END -->
    <!-- TAC_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from TAC_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ TAC_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ TAC_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ TAC_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      TAC_IN_WORKTREE: "true",
      TAC_WORKTREE_NAME: "tac-pr-432",
      TAC_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("tac-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="tac-worktree-name"');
    expect(meta).toContain('content="tac-pr-432"');
    expect(meta).toContain('name="tac-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="tac-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="tac-worktree-name"');
  });
});
