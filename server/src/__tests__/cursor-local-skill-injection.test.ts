...
import { ensureCursorSkillsInjected } from "@theagentcompany/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing The Agent Company skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("tac-cursor-skills-src-");
    const skillsHome = await makeTempDir("tac-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "tac");
    await createSkillDir(skillsDir, "tac-create-agent");
    ...
    const injectedA = path.join(skillsHome, "tac");
    const injectedB = path.join(skillsHome, "tac-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "tac")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "tac-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "tac"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "tac-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("tac-cursor-preserve-src-");
    const skillsHome = await makeTempDir("tac-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "tac");
    ...
    expect((await fs.lstat(path.join(skillsHome, "tac-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("tac-cursor-fail-src-");
    const skillsHome = await makeTempDir("tac-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
