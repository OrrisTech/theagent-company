...
describe("agent local JWT", () => {
  const secretEnv = "TAC_AGENT_JWT_SECRET";
  const ttlEnv = "TAC_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "TAC_AGENT_JWT_ISSUER";
  const audienceEnv = "TAC_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "tac";
    process.env[audienceEnv] = "tac-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
