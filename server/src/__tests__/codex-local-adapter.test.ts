...
import { isCodexUnknownSessionError, parseCodexJsonl } from "@theagentcompany/adapter-codex-local/server";
import { parseCodexStdoutLine } from "@theagentcompany/adapter-codex-local/ui";
  ...
  it("parses turn and reasoning lifecycle events", () => {
    ...
    expect(
      parseCodexStdoutLine(
        JSON.stringify({
          type: "item.completed",
          item: { id: "item_1", type: "reasoning", text: "**Preparing to use TAC skill**" },
        }),
        ts,
      ),
    ).toEqual([
      { kind: "thinking", ts, text: "**Preparing to use TAC skill**" },
    ]);
  });
      ...
      parseCodexStdoutLine(
        JSON.stringify({
          ...
          item: {
            id: "item_52",
            type: "file_change",
            changes: [{ path: "/Users/tacuser/project/ui/src/pages/AgentDetail.tsx", kind: "update" }],
            status: "completed",
          },
        }),
        ts,
