...
import type { DeploymentExposure, DeploymentMode } from "@theagentcompany/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "tac" &&
    normalized !== "tac-create-agent" &&
    normalized !== "tac-create-plugin" &&
...
/** Resolve the TAC repo skills directory (built-in / managed skills). */
function resolveTacSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const tacSkillsDir = resolveTacSkillsDir();

  // Build set of TAC-managed skill names
  const tacSkillNames = new Set<string>();
  if (tacSkillsDir) {
    try {
      for (const entry of fs.readdirSync(tacSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) tacSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isTacManaged: tacSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.tacApiUrl)) {
    const legacyTacApiUrl = nonEmptyTrimmedString(input.tacApiUrl);
    if (legacyTacApiUrl) merged.tacApiUrl = legacyTacApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    tacApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.tacApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawTacApiUrl =
    typeof defaults.tacApiUrl === "string"
      ? defaults.tacApiUrl.trim()
      : "";
  if (rawTacApiUrl) {
    try {
      const parsedTacApiUrl = new URL(rawTacApiUrl);
      if (
        parsedTacApiUrl.protocol !== "http:" &&
        parsedTacApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_tac_api_url_protocol",
          level: "warn",
          message: `tacApiUrl must use http:// or https:// (got ${parsedTacApiUrl.protocol}).`
        });
      } else {
        normalized.tacApiUrl = parsedTacApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_tac_api_url_configured",
          level: "info",
          message: `tacApiUrl set to ${parsedTacApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_tac_api_url_invalid",
        level: "warn",
        message: `Invalid tacApiUrl: ${rawTacApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your The Agent Company host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "The Agent Company is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm theagentcompany allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/tac";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/tac-claimed-api-key.json and load TAC_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: tacApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the TAC hostname is reachable and allowed via `pnpm theagentcompany allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this TAC API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/tac/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # The Agent Company OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          tacApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, The Agent Company generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "tacApiUrl": "https://tac-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in The Agent Company before key claim is allowed.
    ...
    - ~/.openclaw/workspace/tac-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/tac-claimed-api-key.json

    And set the TAC_API_KEY and TAC_API_URL in your environment variables as specified here:
    ...
    Then set TAC_API_KEY and TAC_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install The Agent Company skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your TAC_API_URL to the top of your skill and note the path to your TAC_API_URL
    ...
    ${
      ...
      "Ensure The Agent Company is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested TAC base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.tacApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm theagentcompany allowed-hostname <host>
      - then restart The Agent Company and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@tac.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "tac", path: "/api/skills/tac" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "tac-create-agent",
          path: "/api/skills/tac-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              tacApiUrl: req.body.tacApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.tacApiUrl &&
          !persistedDefaults.tacApiUrl
        ) {
          missingPersistedFields.push("tacApiUrl");
        }
