...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@theagentcompany/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  tacUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).tacUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.tacUpgradeContext = context;
