const FAVICON_BLOCK_START = "<!-- TAC_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- TAC_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- TAC_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- TAC_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.TAC_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.TAC_WORKTREE_NAME) ?? nonEmpty(env.TAC_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.TAC_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="tac-worktree-enabled" content="true" />',
    `<meta name="tac-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="tac-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="tac-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
