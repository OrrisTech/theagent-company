...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.TAC_SECRETS_PROVIDER === undefined) {
    process.env.TAC_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.TAC_SECRETS_STRICT_MODE === undefined) {
    process.env.TAC_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.TAC_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.TAC_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.TAC_MIGRATION_PROMPT === "never") return false;
    if (process.env.TAC_MIGRATION_AUTO_APPLY === "true") return true;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set TAC_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@tac.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.TAC_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.TAC_LISTEN_HOST = runtimeListenHost;
  process.env.TAC_LISTEN_PORT = String(listenPort);
  process.env.TAC_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  await new Promise<void>((resolveListen, rejectListen) => {
    ...
    server.listen(listenPort, config.host, () => {
      ...
      if (process.env.TAC_OPEN_ON_LISTEN === "true") {
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.TAC_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "TAC server failed to start");
    process.exit(1);
  });
}
