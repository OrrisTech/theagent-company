 ...
 */
import { pluginManifestV1Schema } from "@theagentcompany/shared";
import type { TacPluginManifestV1 } from "@theagentcompany/shared";
import { PLUGIN_API_VERSION } from "@theagentcompany/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): TacPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as TacPluginManifestV1,
        };
      }
