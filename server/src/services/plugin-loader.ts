 ...
 *    (`~/.tac/plugins/`) and `node_modules` for packages matching
 *    the `tac-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published TAC plugins.
 * Packages matching this pattern are considered TAC plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "tac-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: TacPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.tac/plugins/ local directory
  | "npm"               // npm packages matching tac-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: TacPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the tac-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "tac-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: TacPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover TAC plugins installed as npm packages in the current
   * Node.js environment matching the "tac-plugin-*" naming convention.
   *
   ...
   * the "tacPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a TAC plugin.
   * Throws if the package is a TAC plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<TacPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: TacPluginManifestV1;
    newManifest: TacPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the TAC plugin naming convention.
 * Accepts both the "tac-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @theagentcompany/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "tacPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const tacPlugin = pkgJson["tacPlugin"];
  if (
    tacPlugin !== null &&
    typeof tacPlugin === "object" &&
    !Array.isArray(tacPlugin)
  ) {
    const manifestRelPath = (tacPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: TacPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumTacVersion;
}
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: TacPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as TacPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a TAC plugin (no manifest found).`,
      );
    }
   ...
   * if the package is not a TAC plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasTacPlugin = "tacPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasTacPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<TacPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: TacPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @theagentcompany/shared exports). Run those workers through
