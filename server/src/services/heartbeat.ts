...
import type { BillingType } from "@theagentcompany/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_tacWakeContext";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__tac_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function evaluateSessionCompaction(input: {
    ...
    const handoffMarkdown = [
      "The Agent Company session handoff:",
  ...
  async function executeRun(runId: string) {
    ...
    context.tacWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.tacRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.tacRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.tacSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.tacSessionRotationReason = sessionCompaction.reason;
      context.tacPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.tacSessionHandoffMarkdown;
      delete context.tacSessionRotationReason;
      delete context.tacPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.tacRuntimeServices = runtimeServices;
        context.tacRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[tac] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected TAC_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.tacRuntimeServices = combinedRuntimeServices;
        context.tacRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[tac] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
