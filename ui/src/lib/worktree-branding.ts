...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("tac-worktree-enabled") !== "true") return null;

  const name = readMetaContent("tac-worktree-name");
  const color = normalizeHexColor(readMetaContent("tac-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("tac-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
