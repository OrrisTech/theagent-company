...
type TacSprite = (typeof TAC_SPRITES)[number];
...
function spriteSize(sprite: TacSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = TAC_SPRITES[(baseCol + baseRow) % TAC_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = TAC_SPRITES[Math.floor(Math.random() * TAC_SPRITES.length)]!;
