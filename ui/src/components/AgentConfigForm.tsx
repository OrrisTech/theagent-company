...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId) throw new Error(t("agentConfigForm.selectCompanyToCreateSecrets"));
  ...
  const uploadMarkdownImage = useMutation({
    mutationFn: async ({ file, namespace }: { file: File; namespace: string }) => {
      if (!selectedCompanyId) throw new Error(t("agentConfigForm.selectCompanyToUploadImages"));
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error(t("agentConfigForm.selectCompanyToTestEnvironment"));
      }
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {isDirty && !props.hideInlineSave && (
        <div className="sticky top-0 z-10 flex items-center justify-end px-4 py-2 bg-background/90 backdrop-blur-sm border-b border-primary/20">
          <div className="flex items-center gap-3">
            ...
            >
              {!isCreate && props.isSaving ? t("agentConfigFormExtra.saving") : t("common.save")}
            </Button>
          </div>
        </div>
      )}
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("teamMember.tabs.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("teamMember.tabs.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("teamMember.identity.name")} hint={help.name}>
              <DraftInput
            ...
            <Field label={t("teamMember.organization.title")} hint={help.title}>
              <DraftInput
            ...
            <Field label={t("teamMember.tabs.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
            ...
            {isLocal && (
              <>
                <Field label={t("pages.workflows.promptConfig.prompt")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("agentConfigForm.promptTemplateIsReplayedOnEveryHeartbeatKeepItComp")}
                </div>
              </>
            )}
          </div>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("agentConfigForm.adapter")}</h3>
          ...
          >
            {testEnvironment.isPending ? t("onboardingWizard.testing") : t("agentConfigFormExtra.testEnvironment")}
          </Button>
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {testEnvironment.error && (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              {testEnvironment.error instanceof Error
                ...
                : t("agentConfigFormExtra.environmentTestFailed")}
            </div>
          )}
          ...
          {/* Working directory */}
          {isLocal && (
            <Field label={t("agentConfigForm.workingDirectory")} hint={help.cwd}>
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agentConfigForm.permissionsAmpConfiguration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agentConfigForm.permissionsAmpConfiguration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("pages.workflows.cliConfig.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
              ...
              {fetchedModelsError && (
                <p className="text-xs text-destructive">
                  {fetchedModelsError instanceof Error
                    ...
                    : t("agentConfigFormExtra.failedToLoadAdapterModels")}
                </p>
              )}
              ...
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agentConfigForm.bootstrapPromptIsLegacyAndWillBeRemovedInAFutureRe")}
                  </div>
                </>
              )}
              {adapterType === "claude_local" && (
              ...
              <Field label={t("agentConfigForm.environmentVariables")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              ...
              numberLabel={t("agentConfigFormExtra.sec")}
              numberPrefix={t("agentConfigFormExtra.runHeartbeatEvery")}
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  const { t } = useTranslation();
  const statusLabel =
    result.status === "pass" ? t("agentConfigFormExtra.passed") : result.status === "warn" ? t("agentConfigFormExtra.warnings") : t("onboardingWizard.failed");
  const statusClass =
    result.status === "pass"
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    try {
      ...
      setSealError(err instanceof Error ? err.message : t("agentConfigFormExtra.failedToCreateSecret"));
    }
  }
        ...
        return (
          <div key={i} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("agentConfigForm.key")}
              value={row.key}
            ...
            >
              <option value="plain">{t("agentConfigForm.plain")}</option>
              <option value="secret">{t("agentConfigForm.secret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">{t("agentConfigForm.selectSecret")}</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title={t("agentConfigForm.createSecretFromCurrentPlainValue")}
                >
                  {t("agentConfigForm.new")}
                </button>
              </>
            ) : (
              <>
                ...
                <button
                  ...
                  onClick={() => sealRow(i)}
                  disabled={!row.key.trim() || !row.plainValue}
                  title={t("agentConfigForm.storeValueAsSecretAndReplaceWithReference")}
                >
                  {t("agentConfigForm.seal")}
                </button>
              </>
            )}
            {!isTrailing ? (
...
function ModelDropdown({
  ...
  return (
    <Field label={t("teamMember.engine.model")} hint={help.model}>
      <Popover
        open={open}
        ...
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? t("agentConfigForm.default") : required ? t("onboardingWizard.selectModelRequired") : t("agentConfigFormExtra.selectModel"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {filteredModels.length === 0 && (
              <p className="px-2 py-1.5 text-xs text-muted-foreground">{t("agentConfigForm.noModelsFound")}</p>
            )}
          </div>
        </PopoverContent>
      </Popover>
