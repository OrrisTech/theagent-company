...
import { AGENT_ROLE_LABELS, type Agent, type AgentRuntimeState } from "@theagentcompany/shared";
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <PropertyRow label={t("teamMember.organization.role")}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("teamMember.organization.title")}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("collaboration.session")}>
        ...
        {agent.reportsTo && (
          <PropertyRow label={t("teamMember.organization.reportsTo")}>
            {reportsToAgent ? (
