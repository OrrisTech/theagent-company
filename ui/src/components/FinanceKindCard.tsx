  ...
  return (
    <Card>
      <CardHeader className="px-4 pt-4 pb-1">
        <CardTitle className="text-base">{t("financeKindCard.financialEventMix")}</CardTitle>
        <CardDescription>{t("financeKindCard.accountLevelChargesGroupedByEventKind")}</CardDescription>
      </CardHeader>
      <CardContent className="space-y-2 px-4 pb-4 pt-3">
        {rows.length === 0 ? (
          <p className="text-sm text-muted-foreground">{t("financeKindCard.noFinanceEventsInThisPeriod")}</p>
        ) : (
          rows.map((row) => (
            <div
              key={row.eventKind}
