...
const EXECUTION_WORKSPACE_OPTIONS = [
  { value: "shared_workspace", labelKey: "issueProperties.projectDefault" },
  { value: "isolated_workspace", labelKey: "issueProperties.newIsolatedWorkspace" },
  { value: "reuse_existing", labelKey: "issueProperties.reuseExistingWorkspace" },
] as const;
  ...
  return (
    <div className={cn("flex items-start gap-1 group", className)}>
      ...
      <button
        ...
        onClick={handleCopy}
        title={copied ? t("issueProperties.copied") : t("issueProperties.copyToClipboard")}
      >
...
export function IssueProperties({ issue, onUpdate, inline }: IssuePropertiesProps) {
  ...
  const labelsTrigger = (issue.labels ?? []).length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("issueProperties.noLabels")}</span>
    </>
  );
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder={t("issueProperties.newLabel")}
            value={newLabelName}
        ...
        >
          ...
          {createLabel.isPending ? t("issueProperties.creating") : t("issueProperties.createLabel")}
        </button>
      </div>
    </>
  );
    ...
    <>
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? t("issueProperties.assignToName", { name: creatorUserLabel }) : t("issueProperties.assignToRequester")}
          </button>
        )}
        {sortedAgents
          .filter((a) => {
  ...
  const projectTrigger = issue.projectId ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("issueProperties.noProject")}</span>
    </>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("pages.workflows.status")}>
          <StatusIcon
            status={issue.status}
        ...
        <PropertyRow label={t("newIssueDialog.priority")}>
          <PriorityIcon
        ...
        <PropertyPicker
          inline={inline}
          label={t("newIssueDialog.assignee")}
          open={assigneeOpen}
              ...
              {issue.currentExecutionWorkspace && (
                <div className="text-[11px] text-muted-foreground space-y-0.5">
                  ...
                  {issue.currentExecutionWorkspace.branchName && (
                    <CopyableValue value={issue.currentExecutionWorkspace.branchName} label={t("agentDetail.branch")} className="text-[11px]" />
                  )}
                  {issue.currentExecutionWorkspace.repoUrl && (
                    <CopyableValue value={issue.currentExecutionWorkspace.repoUrl} label={t("issueProperties.repo")} mono className="text-[11px]" />
                  )}
                </div>
              )}
      ...
      <div className="space-y-1">
        ...
        {issue.startedAt && (
          <PropertyRow label={t("pages.workflows.startedAt")}>
            <span className="text-sm">{formatDate(issue.startedAt)}</span>
          </PropertyRow>
        )}
        {issue.completedAt && (
          <PropertyRow label={t("pages.teamCollab.completed")}>
            <span className="text-sm">{formatDate(issue.completedAt)}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("projectProperties.created")}>
          <span className="text-sm">{formatDate(issue.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("projectProperties.updated")}>
