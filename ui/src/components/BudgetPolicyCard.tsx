...
import type { BudgetPolicySummary } from "@theagentcompany/shared";
...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: (key: string) => string) {
  return windowKind === "lifetime" ? t("budgetPolicyCard.lifetimeBudget") : t("budgetPolicyCard.monthlyUtcBudget");
}
...
export function BudgetPolicyCard({
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("budgetPolicyCard.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("budgetPolicyCardExtra.percentOfLimit", { percent: summary.utilizationPercent }) : t("budgetPolicyCardExtra.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("overview.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("budgetPolicyCardExtra.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budgetPolicyCardExtra.softAlertAt", { percent: summary.warnPercent })}{summary.paused && summary.pauseReason ? t("budgetPolicyCardExtra.pauseReason", { reason: summary.pauseReason }) : ""}
        </div>
      </div>
    </div>
  ) : (
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("budgetPolicyCard.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("budgetPolicyCardExtra.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budgetPolicyCard.executionPausedProject")
          : t("budgetPolicyCard.heartbeatsPausedScope")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("budgetPolicyCard.saving") : summary.amount > 0 ? t("budgetPolicyCard.updateBudget") : t("budgetPolicyCard.setBudget")}
      </Button>
    </div>
  ) : null;
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          ...
          >
            ...
            {summary.paused ? t("budgetPolicyCard.paused") : summary.status === "warning" ? t("budgetPolicyCard.warning") : summary.status === "hard_stop" ? t("budgetPolicyCard.hardStop") : t("budgetPolicyCard.healthy")}
          </div>
        </div>
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("budgetPolicyCard.enterAValidNonNegativeDollarAmount")}</p>
        ) : null}
      </div>
    );
  }
