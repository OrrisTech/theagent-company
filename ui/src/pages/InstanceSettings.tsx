...
import type { InstanceSchedulerHeartbeatAgent } from "@theagentcompany/shared";
...
export function InstanceSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: t("instanceSettings.instanceSettingsBreadcrumb") },
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instanceSettings.loadingSchedulerHeartbeats")}</div>;
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("instanceSettings.agentsWithATimerHeartbeatEnabledAcrossAllOfYourCom")}
        </p>
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instanceSettings.noHeartbeatsMatch")}
        />
      ) : (
