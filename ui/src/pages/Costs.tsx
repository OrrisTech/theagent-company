  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        ...
        <CardDescription>
          {t("costs.accountLevelChargesThatDoNotMapToASingleInferenceR")}
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          ...
          subtitle={t("costs.refundsOffsetsAndCreditReturns")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("costs.net")}
          value={formatCents(netCents)}
          subtitle={t("costs.debitMinusCreditForTheSelectedPeriod")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("costs.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("costs.estimatedDebitsThatAreNotYetInvoiceAuthoritative")}
          icon={Coins}
        />
      </CardContent>
    </Card>
    ...
    <div className="space-y-6">
      <div className="space-y-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h1 className="text-3xl font-semibold tracking-tight">{t("sidebar.costs")}</h1>
                <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                  {t("costs.inferenceSpendPlatformFeesCreditsAndLiveQuotaWindo")}
                </p>
            </div>
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          <TabsTrigger value="overview">{t("pages.overview.title")}</TabsTrigger>
        ...
        <TabsContent value="overview" className="mt-4 space-y-4">
          {showCustomPrompt ? (
            <p className="text-sm text-muted-foreground">{t("costs.selectAStartAndEndDateToLoadData")}</p>
          ) : showOverviewLoading ? (
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>
                      {t("costs.requestScopedInferenceSpendForTheSelectedPeriod")}
                    </CardDescription>
                  </CardHeader>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>{t("costs.whatEachAgentConsumedInTheSelectedPeriod")}</CardDescription>
                  </CardHeader>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>{t("costs.runCostsAttributedThroughProjectLinkedIssues")}</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs.noProjectAttributedRunCostsYet")}</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage={t("costs.noFinanceEventsYetAddAccountLevelChargesOnceBiller")} />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  ...
                  <CardDescription>
                    {t("costs.hardStopSpendLimitsForAgentsAndProjectsProviderSub")}
                  </CardDescription>
                </CardHeader>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t("costs.resolveHardStopsHereByRaisingTheBudgetOrExplicitly")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costsExtra.companyWideMonthlyPolicy")
                            : scopeType === "agent"
                              ? t("costsExtra.recurringMonthlySpendPolicies")
                              : t("costsExtra.lifetimeSpendPolicies")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      {t("costs.noBudgetPoliciesYetSetAgentAndProjectBudgetsFromTh")}
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.noCostEventsInThisPeriod")}</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs.noBillableEventsInThisPeriod")}</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>{t("costs.accountLevelFinancialEventsGroupedByWhoChargedOrCr")}</CardDescription>
                    </CardHeader>
