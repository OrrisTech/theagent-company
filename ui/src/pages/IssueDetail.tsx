...
import type { ActivityEvent } from "@theagentcompany/shared";
import type { Agent, IssueAttachment } from "@theagentcompany/shared";
...
export function IssueDetail() {
  ...
  const uploadAttachment = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("common.uploadFailed"));
    },
  });
  ...
  const deleteAttachment = useMutation({
    ...
    onError: (err) => {
      setAttachmentError(err instanceof Error ? err.message : t("common.deleteFailed"));
    },
  });

  useEffect(() => {
    const titleLabel = issue?.title ?? issueId ?? t("issueDetail.issue");
    setBreadcrumbs([
      sourceBreadcrumb,
  ...
  const copyIssueToClipboard = async () => {
    ...
    setCopied(true);
    pushToast({ title: t("common.copiedToClipboard"), tone: "success" });
  ...
  const attachmentUploadButton = (
    <>
      ...
      >
        ...
        {uploadAttachment.isPending || importMarkdownDocument.isPending ? t("issueDetail.uploadingAttachment") : t("issueDetail.uploadAttachment")}
      </Button>
    </>
  );
    ...
    <div className="max-w-2xl space-y-6">
      ...
      {issue.hiddenAt && (
        <div className="flex items-center gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-sm text-destructive">
          ...
          {t("issueDetail.thisIssueIsHidden")}
        </div>
      )}
      ...
      >
        <div className="flex items-center justify-between gap-2">
          <h3 className="text-sm font-medium text-muted-foreground">{t("newIssueDialog.attachments")}</h3>
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        ...
        <TabsContent value="comments">
          <CommentThread
            ...
            agentMap={agentMap}
            draftKey={`tac:issue-comment-draft:${issue.id}`}
            enableReassign
        ...
        <TabsContent value="activity">
          {!activity || activity.length === 0 ? (
            <p className="text-xs text-muted-foreground">{t("issueDetail.noActivityYet")}</p>
          ) : (
