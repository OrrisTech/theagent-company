  ...
  const statusVariant =
    plugin.status === "ready"
      ...
      : plugin.status === "error"
        ? "destructive"
        : "secondary";
  const pluginDescription = plugin.manifestJson.description || t("pluginSettingsExtra.noDescriptionProvided");
            ...
            <section className="space-y-5">
              ...
              <div className="grid gap-8 lg:grid-cols-[minmax(0,1.4fr)_minmax(220px,0.8fr)]">
                <div className="space-y-2">
                  <h3 className="text-sm font-medium text-muted-foreground">{t("pages.workflows.workflowDescription")}</h3>
                  <p className="text-sm leading-6 text-foreground/90">{pluginDescription}</p>
                </div>
                <div className="space-y-4 text-sm">
                  <div className="space-y-1.5">
                    <h3 className="font-medium text-muted-foreground">{t("pages.teamCollab.author")}</h3>
                        ...
                        {dashboardData.worker ? (
                          <div className="grid grid-cols-2 gap-3 text-sm">
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pages.workflows.status")}</span>
                              <Badge variant={dashboardData.worker.status === "running" ? "default" : "secondary"}>
                                {dashboardData.worker.status}
                              </Badge>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">{t("pluginSettings.pid")}</span>
              ...
              {recentLogs && recentLogs.length > 0 ? (
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>{t("pluginSettingsExtra.lastNLogEntries", { count: recentLogs.length })}</CardDescription>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-3 text-sm text-muted-foreground">
                  ...
                  <div className="flex justify-between gap-3">
                    <span>{t("pages.workflows.version")}</span>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-1.5">
                    ...
                    {t("teamMember.organization.permissions")}
                  </CardTitle>
                </CardHeader>
                <CardContent>
...
interface PluginConfigFormProps {
  ...
  isLoading?: boolean;
  /** Current plugin lifecycle status — t("pluginSettings.testConfiguration") only available when `ready`. */
  pluginStatus?: string;
...
/**
 * Inner component that manages form state, validation, save, and t("pluginSettings.testConfiguration")
...
function PluginConfigForm({ pluginId, schema, initialValues, isLoading, pluginStatus, supportsConfigTest }: PluginConfigFormProps) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      setSaveMessage({ type: "success", text: t("pluginSettingsExtra.configurationSaved") });
      setTestResult(null);
    ...
    onError: (err: Error) => {
      setSaveMessage({ type: "error", text: err.message || t("pluginSettingsExtra.failedToSaveConfiguration") });
    },
  });
      ...
      <div className="flex items-center gap-2 pt-2">
        ...
        >
          {saveMutation.isPending ? (
            ...
            t("pluginSettingsExtra.saveConfiguration")
          )}
        </Button>
        {pluginStatus === "ready" && supportsConfigTest && (
          ...
          >
            {testMutation.isPending ? (
              ...
              t("pluginSettingsExtra.testConfiguration")
            )}
          </Button>
        )}
      </div>
