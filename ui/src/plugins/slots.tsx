 ...
 *
 * Plugin bundles are built with `external: ["@theagentcompany/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__tacPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__tacPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__tacPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__tacPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@theagentcompany/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@theagentcompany/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@theagentcompany/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@theagentcompany/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@theagentcompany/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__tacPluginBridge__) {
 ...
 * self-register via `window.tacPlugins.registerReactComponent()`.
 ...
 * of `@theagentcompany/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
