...
const JWT_SECRET_ENV_KEY = "TAC_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# The Agent Company environment variables",
    "# Generated by TAC CLI commands",
...
export function resolveTacEnvFile(configPath?: string): string {
...
export function loadTacEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeTacEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readTacEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeTacEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeTacEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readTacEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeTacEnvEntries(next, filePath);
  return next;
}
