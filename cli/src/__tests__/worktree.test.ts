...
function buildSourceConfig(): TacConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("tac-pr-432")).toBe(
      path.resolve(os.homedir(), "tac-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("tac/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://tac.example", 3110)).toBe("https://tac.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/tac-feature",
      homeDir: "/tmp/tac-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/tac-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/tac-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.TAC_HOME).toBe(path.resolve("/tmp/tac-worktrees"));
    expect(env.TAC_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.TAC_IN_WORKTREE).toBe("true");
    expect(env.TAC_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.TAC_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export TAC_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tac-worktree-secrets-"));
    const originalInlineMasterKey = process.env.TAC_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.TAC_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.TAC_SECRETS_MASTER_KEY;
      delete process.env.TAC_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.TAC_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.TAC_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.TAC_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          TAC_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tac-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.TAC_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.TAC_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".tac-worktrees"),
      });

      const envPath = path.join(repoRoot, ".tac", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("TAC_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("TAC_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/TAC_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.TAC_AGENT_JWT_SECRET;
      } else {
        process.env.TAC_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });

  it("defaults the seed source config to the current repo-local The Agent Company config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tac-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".tac", "config.json");
    const originalCwd = process.cwd();
    const originalTacConfig = process.env.TAC_CONFIG;

    try {
      fs.mkdirSync(path.dirname(localConfigPath), { recursive: true });
      fs.writeFileSync(localConfigPath, JSON.stringify(buildSourceConfig()), "utf8");
      delete process.env.TAC_CONFIG;
      process.chdir(repoRoot);

      expect(fs.realpathSync(resolveSourceConfigPath({}))).toBe(fs.realpathSync(localConfigPath));
    } finally {
      process.chdir(originalCwd);
      if (originalTacConfig === undefined) {
        delete process.env.TAC_CONFIG;
      } else {
        process.env.TAC_CONFIG = originalTacConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tac-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/tac",
        targetRepoRoot: "/Users/example/tac-pr-432",
        workspaceCwd: "/Users/example/tac",
      }),
    ).toBe("/Users/example/tac-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/tac",
        targetRepoRoot: "/Users/example/tac-pr-432",
        workspaceCwd: "/Users/example/tac/packages/db",
      }),
    ).toBe("/Users/example/tac-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tac-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "tac-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "tac-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("tac-make-test", {
        seed: false,
        home: path.join(tempRoot, ".tac-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".tac", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".tac", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
