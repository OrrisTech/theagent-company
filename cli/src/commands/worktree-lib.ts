...
export const DEFAULT_WORKTREE_HOME = "~/.tac-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".tac");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "tac",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    TAC_HOME: paths.homeDir,
    TAC_INSTANCE_ID: paths.instanceId,
    TAC_CONFIG: paths.configPath,
    TAC_CONTEXT: paths.contextPath,
    TAC_IN_WORKTREE: "true",
    ...(branding?.name ? { TAC_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { TAC_WORKTREE_COLOR: branding.color } : {}),
  };
}
