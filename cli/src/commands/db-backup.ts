...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@theagentcompany/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://tac:tac@127.0.0.1:${port}/tac`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printTacCliBanner();
  p.intro(pc.bgCyan(pc.black(" tac db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveTacInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "tac";
