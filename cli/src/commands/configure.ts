...
function defaultConfig(): TacConfig {
...
export async function configure(opts: {
  ...
  printTacCliBanner();
  p.intro(pc.bgCyan(pc.black(" tac configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `theagentcompany onboard` first.");
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because TAC_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
