...
import { ensureAgentJwtSecret, loadTacEnvFile, mergeTacEnvEntries, readTacEnvEntries, resolveTacEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.TAC_CONFIG;
...
const WORKTREE_NAME_PREFIX = "tac-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.TAC_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.TAC_WORKTREE_START_POINT) ?? undefined;
}
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.tac"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: TacConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://tac:tac@127.0.0.1:${port}/tac`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: TacConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.TAC_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.TAC_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.TAC_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.TAC_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: TacConfig;
  targetConfig: TacConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveTacEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readTacEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://tac:tac@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "tac");
    const targetConnectionString = `postgres://tac:tac@127.0.0.1:${targetHandle.port}/tac`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readTacEnvEntries(resolveTacEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.TAC_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.TAC_AGENT_JWT_SECRET);
  mergeTacEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { TAC_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadTacEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run The Agent Company inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printTacCliBanner();
  p.intro(pc.bgCyan(pc.black(" theagentcompany worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printTacCliBanner();
  p.intro(pc.bgCyan(pc.black(" theagentcompany worktree:make ")));
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printTacCliBanner();
  p.intro(pc.bgCyan(pc.black(" theagentcompany worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveTacEnvFile(configPath);
  const envEntries = readTacEnvEntries(envPath);
  const out = {
    TAC_CONFIG: configPath,
    ...(envEntries.TAC_HOME ? { TAC_HOME: envEntries.TAC_HOME } : {}),
    ...(envEntries.TAC_INSTANCE_ID ? { TAC_INSTANCE_ID: envEntries.TAC_INSTANCE_ID } : {}),
    ...(envEntries.TAC_CONTEXT ? { TAC_CONTEXT: envEntries.TAC_CONTEXT } : {}),
    ...envEntries,
  };
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local TAC instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated TAC instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with tac- if needed (created at ~/tac-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: TAC_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: TAC_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source TAC_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local TAC instance")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with tac- if needed")
