...
const DEFAULT_AGENT_JWT_ISSUER = "tac";
const DEFAULT_AGENT_JWT_AUDIENCE = "tac-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveTacInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveTacInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" tac env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: TacConfig | null = null;
...
function collectDeploymentEnvRows(config: TacConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrl =
    process.env.TAC_PUBLIC_URL ??
    process.env.TAC_AUTH_PUBLIC_BASE_URL ??
  ...
  const publicUrlSource: EnvSource =
    process.env.TAC_PUBLIC_URL
      ? "env"
      : process.env.TAC_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from TAC_PUBLIC_URL when possible)",
    },
    {
      key: "TAC_AGENT_JWT_TTL_SECONDS",
      value: process.env.TAC_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.TAC_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "TAC_AGENT_JWT_ISSUER",
      value: process.env.TAC_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.TAC_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "TAC_AGENT_JWT_AUDIENCE",
      value: process.env.TAC_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.TAC_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "TAC_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.TAC_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "TAC_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.TAC_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "TAC_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.TAC_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "TAC_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.TAC_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "TAC_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.TAC_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "TAC_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.TAC_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "TAC_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.TAC_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "TAC_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.TAC_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "TAC_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.TAC_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "TAC_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.TAC_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.TAC_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "TAC_CONFIG",
      value: process.env.TAC_CONFIG ?? configPath,
      source: process.env.TAC_CONFIG ? "env" : "default",
      required: false,
