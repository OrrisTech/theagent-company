...
import type { Agent } from "@theagentcompany/shared";
import {
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export TAC_API_URL='${escaped(input.apiBase)}'`,
    `export TAC_COMPANY_ID='${escaped(input.companyId)}'`,
    `export TAC_AGENT_ID='${escaped(input.agentId)}'`,
    `export TAC_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local The Agent Company skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing The Agent Company skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveTacSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local The Agent Company skills directory. Expected ./skills in the repo checkout.",
              );
            }
