...
import { ApiRequestError, TacApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to The Agent Company config file")
    .option("-d, --data-dir <path>", "TAC data directory root (isolates state from ~/.tac)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the TAC API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.TAC_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.TAC_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.TAC_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set TAC_COMPANY_ID, or set context profile companyId via `theagentcompany context set`.",
    );
  }

  const api = new TacApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.TAC_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.TAC_SERVER_PORT || "");
