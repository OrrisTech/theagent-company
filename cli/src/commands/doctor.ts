...
export async function doctor(opts: {
  ...
  printTacCliBanner();
  p.intro(pc.bgCyan(pc.black(" tac doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "Run `theagentcompany configure --section database` or `theagentcompany onboard`",
    };
...
async function runRepairableCheck(input: {
  ...
  loadTacEnvFile(input.configPath);
