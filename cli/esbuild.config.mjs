/**
 * esbuild configuration for building the theagentcompany CLI for npm.
 *
 * Bundles all workspace packages (@theagentcompany/*) into a single file.
...
// to npm and resolved at runtime (e.g. @theagentcompany/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@theagentcompany/")) {
      externals.add(name);
    }
  }
